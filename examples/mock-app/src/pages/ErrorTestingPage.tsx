...
export function ErrorTestingPage() {
  ...
  const source = errorScenario === 'none' ? 'product' : `product?error=${errorScenario}`;
    ...
    <div className="space-y-6">
        ...
        <div className="space-y-3">
          <label className="flex items-center space-x-3">
            <input
              ...
              onChange={(e) => handleErrorScenarioChange(e.target.value as ErrorScenario)}
              className="text-blue-600"
            />
            <span className="text-green-700 font-medium">✅ Normal Operation</span>
          ...
          <label className="flex items-center space-x-3">
            <input
              ...
              className="text-blue-600"
            />
            <span className="text-red-700 font-medium">🔥 Network Error (500)</span>
          ...
          <label className="flex items-center space-x-3">
            <input
              ...
              className="text-blue-600"
            />
            <span className="text-orange-700 font-medium">🔒 Authorization Error (401)</span>
          ...
          <label className="flex items-center space-x-3">
            <input
              ...
              className="text-blue-600"
            />
            <span className="text-purple-700 font-medium">❓ Not Found Error (404)</span>
      ...
      <div className="bg-gray-50 p-4 rounded-lg">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            ...
            <span className={`ml-1 ${table.isLoading ? 'text-blue-600' : 'text-gray-500'}`}>
              {table.isLoading ? 'Yes' : 'No'}
            </span>
          </div>
          <div>
            <strong>Fetching:</strong> 
            <span className={`ml-1 ${table.isFetching ? 'text-blue-600' : 'text-gray-500'}`}>
              {table.isFetching ? 'Yes' : 'No'}
            </span>
          </div>
          <div>
            <strong>Error:</strong> 
            <span className={`ml-1 ${table.error ? 'text-red-600' : 'text-green-600'}`}>
              {table.error ? 'Yes' : 'No'}
            </span>
          </div>
          <div>
      ...
      {table.isLoading && (
        <div className="bg-blue-50 border border-blue-200 rounded p-6">
          <div className="flex items-center">
            ...
            <span className="ml-3 text-blue-800">Loading products for error scenario: <strong>{errorScenario}</strong></span>
          </div>
        </div>
      )}
      ...
      {table.error && (
        <div className="bg-red-50 border border-red-200 rounded p-6">
          ...
          <p className="text-red-600 text-sm mt-2">
            This error was triggered by the "<strong>{errorScenario}</strong>" scenario.
          </p>
          <button 
            onClick={() => table.refetch()}
      ...
      {!table.isLoading && !table.error && table.rows.length > 0 && (
        <div className="bg-green-50 border border-green-200 rounded p-6">
          ...
          {/* Sample Data Table */}
          <div className="mt-4">
            <h4 className="text-sm font-medium text-green-800 mb-2">Sample Data (First 3 rows):</h4>
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="bg-green-100">
                    <th className="px-3 py-1 text-left text-green-800">Name</th>
                    <th className="px-3 py-1 text-left text-green-800">Category</th>
                    <th className="px-3 py-1 text-left text-green-800">Price</th>
                    <th className="px-3 py-1 text-left text-green-800">Stock</th>
                  </tr>
                </thead>
                <tbody className="bg-green-50">
                  {table.rows.slice(0, 3).map((product, index) => (
                    <tr key={index}>
                      <td className="px-3 py-1 text-green-700">{product.name}</td>
                      <td className="px-3 py-1 text-green-700">{product.category}</td>
      ...
      {!table.isLoading && !table.error && table.rows.length === 0 && (
        <div className="bg-yellow-50 border border-yellow-200 rounded p-6">
          ...
          <p className="text-yellow-700">
            No products were returned. This might be normal depending on filters or could indicate an issue.
          </p>
        </div>
      )}
      ...
      <div className="bg-blue-50 border border-blue-200 rounded p-4">
        ...
        <ul className="text-blue-700 text-sm space-y-1 list-disc list-inside">
          <li>Switch between scenarios to test different error handling paths</li>
          <li>Check the browser's Network tab to see the actual HTTP requests</li>
